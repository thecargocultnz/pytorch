...
def package_pt2(
    ...
    *,
    ...
    assert (isinstance(f, (io.IOBase, IO)) and f.writable() and f.seekable()) or (
        isinstance(f, (str, os.PathLike)) and os.fspath(f).endswith(".pt2")
    ), f"Expect archive file to be a file ending in .pt2, or is a buffer. Instead got {f}"
...
def load_pt2(
    ...
    *,
    ...
    assert (isinstance(f, (io.IOBase, IO)) and f.readable() and f.seekable()) or (
        isinstance(f, (str, os.PathLike)) and os.fspath(f).endswith(".pt2")
    ), f"Unable to load package. f must be a buffer or a file ending in .pt2. Instead got {f}"
